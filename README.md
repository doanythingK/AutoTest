...
WPF 화면의 `로그 폴더 열기`, `최근 로그 열기`, `실패 자료 열기` 버튼으로 실행 로그 파일과 실패 화면 저장 폴더를 바로 열 수 있습니다.
...
- 차변, 매출구분, 전자(세금)계산서 발송구분 드롭다운은 화면 라벨이 축약되어도 대체 라벨명으로 찾고, 옵션은 공백/괄호/구분자를 제거해 비교한다. 선택 후에는 ERP 조회/처리 표시가 사라질 때까지 짧게 기다린다.
